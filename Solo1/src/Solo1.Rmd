...
```{r}
map_int(df_nums,~sum(is.na(.x))) %>% sort() %>% tail() %>% barchart()
```
...
```{r}
map_dbl(df_nums_adj,~max(.x))
map_dbl(df_nums_adj,~sd(.x))
```
...
```{r}
wssplot <- function(numsub, nc=15, seed=10) {
...
pamFits$pamFits <- map(pamFits$k_pam, ~pam(df_nums_adj, k = .x, metric = 'manhattan'))
...
mclustFits$mclustFits <- map(mclustFits$mclust_clusters, ~Mclust(df_nums_adj, G = .x))
mclustFits$bic <- map_dbl(mclustFits$mclustFits, ~.x[['bic']])
mclustFits$loglik <- map_dbl(mclustFits$mclustFits, ~.x[['loglik']])
# plot(fit,data=df_nums_adjscaled, what="density") # plot results
mclustFits
```
...
```{r}
summary(mclustFits[[3,'mclustFits']])
```

```{r}
mclust_clusters <- mclustFits[[3,'mclustFits']]$classification
